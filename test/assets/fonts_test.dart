...
void main() {
  ...
  test('all app and background entrypoints configure fonts before startup', () {
    expect(File('lib/main.dart').readAsStringSync(),
        contains('Future<void> main() => bootstrap();'));
    const entrypoints = {
      'lib/bootstrap.dart': ['Future<void> bootstrap() async {'],
      'lib/background/bg_service.dart': [
        'Future<bool> onIosBackground(ServiceInstance service) async {',
        'void onStart(ServiceInstance service) async {',
      ],
      'lib/background/widget_handler.dart': [
        'FutureOr<void> backgroundCallback(Uri? data) async {',
        'void workmanagerCallback() {',
      ],
      'lib/background/notification_handler.dart': [
    ...
    for (final entry in entrypoints.entries) {
      ...
      for (final signature in entry.value) {
        expect(source, contains('$signature\n  configureBundledFonts();'),
            reason: '${entry.key}: $signature must disable fetching first');
      }
    }
  });
  ...
  test('font request sites stay covered by the offline rendering test', () {
    ...
    expect(requests, {
      'lib/ui/theme/app_theme.dart': ['nunitoTextTheme', 'nunitoTextTheme'],
      'lib/ui/screens/ls_keycard_screen.dart': ['kodeMono'],
      'lib/ui/screens/ls_scheduled_hibernation_screen.dart': ['kodeMono'],
    });
    final main = File('lib/ui/theme/app_theme.dart').readAsStringSync();
    ...
    expect(File('lib/ui/screens/ls_keycard_screen.dart').readAsStringSync(),
        contains(RegExp(r'GoogleFonts\.kodeMono\(\s*color: Colors.white,\s*fontSize: 28,\s*\)')));
    expect(File('lib/ui/screens/ls_scheduled_hibernation_screen.dart').readAsStringSync(), contains('GoogleFonts.kodeMono()'));
  });
