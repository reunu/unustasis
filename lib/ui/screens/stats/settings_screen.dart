...
import 'package:scooter_core/actions.dart'
    show ApnProblem, checkApn, maxApnLength, lsKeyAutoStandbySeconds, lsKeyHibernateTimer;
import 'package:unustasis/state/vehicle_status.dart';
import 'package:unustasis/ui/widgets/wide_layout.dart';
import 'package:unustasis/ui/screens/stats/log_screen.dart';
...
class _SettingsScreenState extends State<SettingsScreen> with WidgetsBindingObserver {
  ...
  void _syncSession({bool force = false}) {
    ...
    final token = service.connected ? service.connectionToken : null;
    final bool asleep = _scooterAsleep;
    if (!force &&
        _sessionConnected == service.connected &&
        identical(token, _sessionToken) &&
    ...
    final bool sameLink = !force && _sessionConnected == service.connected && identical(token, _sessionToken);
  ...
  bool _isCurrent(int session) =>
      ...
      _service!.connected &&
      _sessionToken != null &&
      identical(_service!.connectionToken, _sessionToken);
  ...
  Future<void> _getKeycardCount(int session) async {
    if (!mounted || !_isCurrent(session)) return;
    try {
      final count = await _service!.actions.countKeycards();
  ...
  List<Widget> alarmItems() {
    ...
    final bool live = service.connected && service.alarmAvailable;
