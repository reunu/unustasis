...
open class FSIShareViewController: UIViewController {
    ...
    // MARK: - Override point
    /// Called on the main queue when all attachments have been collected into
    /// `sharedMedia`. The default implementation immediately saves and opens
    /// the host app. Override to show a confirmation UI first.
    open func onAttachmentsReady() {
    ...
    open func processAttachments() {
        ...
        for (index, provider) in attachments.enumerated() {
            ...
            if provider.isImage {
                provider.loadItem(forTypeIdentifier: UType.image, options: nil) {
            ...
            if provider.isMovie {
                provider.loadItem(forTypeIdentifier: UType.movie, options: nil) {
                    [weak self] data, error in
                    defer { group.leave() }
                    guard let self = self, error == nil else {
                        self?.dismissWithError()
            ...
            if provider.isFile {
                provider.loadItem(forTypeIdentifier: UType.fileURL, options: nil) {
                    [weak self] data, error in
                    defer { group.leave() }
                    guard let self = self, error == nil else {
                        self?.dismissWithError()
            ...
            if provider.isURL {
                provider.loadItem(forTypeIdentifier: UType.url, options: nil) {
                    [weak self] data, error in
                    defer { group.leave() }
                    guard let self = self, error == nil else {
                        self?.dismissWithError()
            ...
            if provider.isText {
                let id =
                    provider.hasItemConformingToTypeIdentifier(UType.plainText)
                    ? UType.plainText
                    : UType.text
            ...
            if provider.isData {
                provider.loadItem(forTypeIdentifier: UType.data, options: nil) {
                    [weak self] data, error in
                    defer { group.leave() }
                    guard let self = self, error == nil else {
                        self?.dismissWithError()
            ...
            if provider.isItem {
                provider.loadItem(forTypeIdentifier: UType.item, options: nil) {
                    [weak self] data, error in
                    defer { group.leave() }
                    guard let self = self, error == nil else {
                        self?.dismissWithError()
    ...
    private func handleTextItem(data: NSSecureCoding?, index: Int, total: Int) {
        if let s = data as? String {
            ...
            sharedMedia.append(
                SharingFile(value: url.absoluteString, thumbnail: nil, duration: nil, type: .url))
        }
    }
        ...
        if let url = data as? URL {
            ...
            if let dst = containerURL()?.appendingPathComponent(filename) {
                if copyFile(at: url, to: dst) {
                    sharedMedia.append(
                        SharingFile(
                            value: dst.absoluteString, mimeType: url.mimeType(), thumbnail: nil,
                            duration: nil, type: .image))
                }
            }
        ...
        if let url = data as? URL {
            ...
            if let dst = containerURL()?.appendingPathComponent(filename) {
                do {
                    ...
                    sharedMedia.append(
                        SharingFile(
                            value: dst.absoluteString, mimeType: "application/octet-stream",
                            thumbnail: nil, duration: nil, type: .file))
                } catch {}
            }
        }
    }
    ...
    private func writeTempImage(_ image: UIImage) -> SharingFile? {
        ...
        do {
            if let d = image.pngData() {
                ...
                return SharingFile(
                    value: decoded, mimeType: "image/png", thumbnail: nil, duration: nil,
                    type: .image)
            }
        } catch {
    ...
    private func redirectToHostApp() {
        loadIds()
        let urlString = "\(kSchemePrefix)-\(hostAppBundleIdentifier)://dataUrl=\(kUserDefaultsKey)"
        log("redirectToHostApp url=\(urlString)")
        guard let url = URL(string: urlString) else {
            log("redirectToHostApp: URL construction failed, falling back to completeAndExit")
            completeAndExit()
            return
        }
        guard let ctx = extensionContext else {
            log("redirectToHostApp: extensionContext is nil!")
            return
        }
        // extensionContext.open() requires a user-gesture callsite (button tap).
        // The completion handler MUST call completeRequest — without it iOS
        // holds the extension alive and never performs the app switch.
        ctx.open(url) { [weak self] success in
            self?.log("extensionContext.open success=\(success)")
    ...
    private func getSharedMediaFile(forVideo: URL) -> SharingFile? {
        ...
        if FileManager.default.fileExists(atPath: thumbnailPath.path) {
            return SharingFile(
                value: forVideo.absoluteString, mimeType: forVideo.mimeType(),
                thumbnail: thumbnailPath.absoluteString, duration: Int(duration), type: .video)
        }
        ...
        let time = CMTime(
            seconds: min(1.0, CMTimeGetSeconds(asset.duration)), preferredTimescale: 600)
        do {
        ...
        return SharingFile(
            value: forVideo.absoluteString, mimeType: forVideo.mimeType(), thumbnail: nil,
            duration: Int(duration), type: .video)
    }
    ...
    private func saveToUserDefaults(data: [SharingFile]) {
        ...
        if let enc = try? JSONEncoder().encode(data) {
            ud?.set(enc, forKey: kUserDefaultsKey)
            ud?.synchronize()
        }
    }
...
extension NSItemProvider {
    ...
    var isItem: Bool { return hasItemConformingToTypeIdentifier(UType.item) }
}
...
class SharingFile: Codable {
    ...
    init(
        value: String, mimeType: String? = nil, thumbnail: String?, duration: Int?,
